function [I_si_vec] = f__synaptic_response_function__nonlinear_regime(time_vec,input_spike_times,I_0,I_si_sat,gamma1,gamma2,gamma3,tau_rise,tau_fall)
...
%             (1-exp(-(time_vec(ind_vec(1:end))-input_spike_times(ii))/tau_rise)).*exp(-(time_vec(ind_vec(1:end))-input_spike_times(ii))/tau_fall);
    ...
    for jj = 1:length(ind_vec)
        ...
        if time_vec(ind_vec(jj))-input_spike_times(ii) <= tau_rise
            ...
            I_si_mat(ind_vec(jj),ii) = f__synaptic_response_prefactor(I_0,I_si_sat,gamma1,gamma2,I_si_vec_temp(ind_vec(ii)-1),tau_rise,tau_fall)*...
