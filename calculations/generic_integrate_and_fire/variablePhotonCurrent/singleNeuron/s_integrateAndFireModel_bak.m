...
tauSPDVec = [50e-9 100e-9 200e-9];%receiver nanowire L/r time constant
tauRef = 50e-9;%refractory period

jPhVec = 1e6*(1:1:10000);%input photon current (photons/sec)
...
IntCVec = nNW*Ispd*([0.01 0.05 0.1]);%critical current of nTron
...
for jj = 1:length(IntCVec)
    for ii = 1:length(tauSPDVec)
        ...
        legendStr = [legendStr sprintf('''IntC = %g nNw Ispd, tauSPD = %g ns''',IntCVec(jj)/nNW/Ispd,tauSPDVec(ii)*1e9) ','];
        hold on
    end
end
...
tauSPDVec = [100e-9 1000e-9];%[50e-9 100e-9 200e-9];%receiver nanowire L/r time constant
tauRef = 50e-9;%refractory period
