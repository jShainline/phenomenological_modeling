...
\captionof{figure}{\label{fig:tau_rise_vs_Isy}Optimized values of $\tau_{\mathrm{rise}}(I_{sy})$ and a linear fit.}
\end{figure}
...
\captionof{figure}{\label{fig:finding_gammas}Error as a function of $\gamma_1$ and $\gamma_2$. (a) $I_{sy} = x$\,\textmu A. (b) $I_{sy} = x$\,\textmu A. (c) $I_{sy} = x$\,\textmu A. (d) $I_{sy} = x$\,\textmu A. Note that we found $\gamma_1$ and $\gamma_2$ by finding decent fits for 11 values of $I_{sy}$ from 34\,\textmu A to 39\,\textmu A with $L_{si} = 50$\,nH and $r_{si} = 1\,\Omega$ and an input rate of synaptic firing events of 40\,MHz, but the values discovered provide good fits to behavior with all values of $I_{si}$, $L_{si}$, $r_{si}$, and input rate tested thus far. When determining $\gamma_1$ and $\gamma_2$, we used $\gamma_3 = 1$.}
\end{figure}
