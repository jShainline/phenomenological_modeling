...
tau_rise =  [1e-9 1.5e-9 2e-9 2.75e-9 3e-9 4.19e-9 5e-9 5.63e-9 6.37e-9 7.33e-9 8.29e-9];
tau_fall = L_si/r_si;%(L_jj+L_si)/r_si;
I_0_a = [290e-9 0.696e-6 1.137e-6 1.650e-6 2.194e-6 2.767e-6 3.369e-6 4.001e-6 4.659e-6 5.365e-6 6.091e-6];
I_si_sat = [10e-6 10e-6 10e-6 10e-6 10e-6 10e-6 10e-6 10e-6 10e-6 11.2e-6 11.4e-6];
gamma1_range = [0 3];%f(I_si) exponent
gamma2_range = [0 3];%f(I_si) exponent
num_gamma = 30;
...
%     figure('OuterPosition',[0 0 scrsz(3) scrsz(4)]);
%     imagesc(gamma1_vec,gamma2_vec,error_cell{ii}.')
%     colorbar
%     set(gca,'YDir','normal')
%     xlabel('\gamma_1','FontSize',fontSize,'FontName','Times')
%     ylabel('\gamma_2','FontSize',fontSize,'FontName','Times')
%     title('error','FontSize',fontSize,'FontName','Times')
%     set(gca,'FontSize',fontSize,'FontName',fontName)
%     k1 = gtext(figureCaptions(1:length(figureCaptions)));
%     set(k1,'FontSize',fontSize_legend,'FontName','Times')
%     % xlim([times(1) times(end)]*1e9)
...
%% plot min gammas vs I_sy

figure('OuterPosition',[0 0 scrsz(3) scrsz(4)]);
[AX,H1,H2] = plotyy(I_sy_vec*1e6,gamma1_vec_optimized,I_sy_vec*1e6,gamma2_vec_optimized,'plot','plot');
